...
def read_server_cfg() -> dict:
    ...
    if not cfg_path.exists():
        return {}
    data, section = {}, None
    with open(cfg_path, encoding="utf-8") as f:          # Bug fix #2
        for line in f:
            line = line.strip()
...
def update_server_cfg(updates: dict) -> tuple[bool, str]:
    """Schreibt Schlüssel in die [SERVER]-Sektion von server_cfg.ini."""
    cfg_path = CFG_DIR / "server_cfg.ini"
    if not cfg_path.exists():
        return False, "server_cfg.ini not found"
    lines = cfg_path.read_text(encoding="utf-8").splitlines()  # Bug fix #2
    new_lines = []
    found_keys: set = set()
...
def write_extra_cfg(updates: dict) -> tuple[bool, str]:
    ...
    while i < len(lines):
        ...
        if matched_key is not None:
            ...
            # Bug fix #4: nur echte Einrückungszeilen überspringen
            i += 1
            while i < len(lines):
                nxt = lines[i]
                if not nxt.startswith("  "):   # Nicht eingerückt → neuer Key oder Leerzeile
                    break
                i += 1
            continue
        new_lines.append(line)
